...
const Button: React.FC<ButtonProps> = ({ disabled, outline, label, onClick, small, icon: Icon }) => {
  return (
    <button
      onClick={onClick} disabled={disabled}
      className={`relative disabled:opacity-70  disabled:cursor-not-allowed rounded-lg
    hover:opacity-80 transition w-full
    ${outline ? 'bg-white border-black text-black' : 'bg-emerald-500 border-emerald-500 text-white'}
